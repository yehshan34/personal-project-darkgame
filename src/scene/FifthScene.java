...
public class FifthScene extends Scene {
    ...
    public FifthScene(int playerNum){
    ...
    public void sceneBegin() {
        ...
        player = new Player(playerNum, 448, 1216);
        glow = new Glow(448, 1216, player);
        key = new Key(1472, 448);
...
//        player.setLastPosition(player.collider().centerX(), player.collider().centerY());
    ...
    public void playerDead(){
        if (deadDelay.count() && player.getState() != Player.PlayerState.ALIVE) {
            deathCount++;
            SceneController.getInstance().change(new TransitionScene(4, playerNum));
        }
    }
    ...
    public void stair(){
        stairArr.forEach(g -> {
            if (g.isCollisionCenter(player) && player.keyCondition()) {
                ...
                SceneController.getInstance().change(new TowerScene(4,playerNum));
            }
        });
    }
    public void monster(){
